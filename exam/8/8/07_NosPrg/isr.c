	...
	if(gcRecvBuf[0]!=0xFF)
    {
		   ...
		   switch(gcRecvBuf[0])
		   {
		        case'r':
					...
					printf("r %d %d\n",gcRecvBuf[1],(gcRecvBuf[2]<<8)|gcRecvBuf[3]);
					mFlag=1;
...
//					uart_sendN(UART_User,gcRecvBuf[5],flashRead);
					printf("a %d %d\n",(gcRecvBuf[1]<<24)|(gcRecvBuf[2]<<16)|(gcRecvBuf[3]<<8)|gcRecvBuf[4],gcRecvBuf[5]);
					mFlag=1;
				...
				case 'w':
					...
					printf("w %d %d %d\n",gcRecvBuf[1],(gcRecvBuf[2]<<8)|gcRecvBuf[3],gcRecvBuf[4]);
					if(mFlag==0)
