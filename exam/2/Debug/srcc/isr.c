...
void UART_User_Handler(void)
{
	...
	//------------------------------------------------------------------
	//接收一个字节
	flag = uart_reN(UART_User,6, buffer); 
	uint8_t hour=(buffer[0]-'0')*10+buffer[1]-'0';
	uint8_t min=(buffer[2]-'0')*10+buffer[3]-'0';
	uint8_t sec=(buffer[4]-'0')*10+buffer[5]-'0';
	if(flag)   //有数据
	{
		printf("%d:%d:%d\n",hour,min,sec);
...
void SYSTICK_USER_Handler()
{
	...
	//wdog_feed();      //看门狗“喂狗”
	if(SysTickCount == 20)SecAdd1(gTime);
	if(SysTickCount==40)SecAdd1(gTime);
	if(SysTickCount==60)SecAdd1(gTime);
	if(SysTickCount==80)SecAdd1(gTime);
	if (SysTickCount >= 100)
	{
		SysTickCount = 0;
		SecAdd1(gTime);
